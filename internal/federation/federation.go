...
// Subprocess- and filesystem-coupled helpers for the federation
// protocol. Pure value types and helpers (Config, ConfigStore,
// ParseUpstream, ResolveConfig, mode/backend constants, errors) live
// in `config.go`, which compiles in any build mode including
// GOOS=js GOARCH=wasm. This file holds the bits that depend on the
// dolt CLI subprocess and on local filesystem state.
